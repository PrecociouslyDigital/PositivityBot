...
function searchImBad(message, lowMessage) {
		...
		if(decLocation != -1) {
			for(i = 0; i < badWords.length; i++) {
				if(lowMessage.substr(decLocation, searchScope).includes(' ' + badWords[i])) {
					message.reply('You are not ' + badWords[i] + '! ' + getAffirmation());
				}
			}
		}
	}
...
function searchImBadFunc(message, lowMessage) {
...
function searchNotGood(message, lowMessage) {
    for(i = 0; i < iAm.length; i++){
        var decLocation = lowMessage.search(iAm[i]);
        if(decLocation != -1) {
...
function searchIsHard(message, lowMessage) {
    var hardLocation = lowMessage.search(" is hard");
    if (hardLocation != -1) {
        for (var i = 1; i < searchScope; i++) {
            if (hardLocation - i == 0) {
                message.reply('Yeah, I understand that ' + lowMessage.substring(hardLocation - i, hardLocation) + ' can be really hard at times. We are here to support you!');
                i = searchScope;
            } else if (lowMessage.charAt(hardLocation - i).match(" ")) {
                message.reply('Yeah, I understand that ' + lowMessage.substring(hardLocation - i + 1, hardLocation) + ' can be really hard at times. We are here to support you!');
                i = searchScope;
            }
        }
    }
...
function runSlashFunctions(message, lowMessage) {
